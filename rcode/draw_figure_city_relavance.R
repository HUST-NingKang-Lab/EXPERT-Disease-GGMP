...
#画出来样本量和准确性关系图
  ...
  for(i in 2:length(disease))
  {
    ...
    #更换分组名称
        ...
        aggregate(x=sum$value, by=list(sum$method,sum$group),mean)

    q <- ggplot(sum,aes(x = method,y = value,colour = group)) +
      geom_boxplot(position=position_dodge(0.6),width = 0.5) +
      geom_hline(aes(yintercept=0.550), colour="#990000", linetype="dashed")+
    ...
    ggsave(paste0("/home/wangnan/GGMP-SGMP/figure/exp_region/city_relevance(mean)/",disease[i],".pdf"),q,height = 6,width = 6)
  }
