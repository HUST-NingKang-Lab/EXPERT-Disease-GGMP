    ...
    for (n in 1:4) 
    {
      for(i in 2:length(disease)) 
      {
        for (j in 1:nrow(city)) 
        {
          ...
          #抽样
          s <- sample(length(metadata[,1]),(length(metadata[,1])*n*2) %/% 10,replace = FALSE)
          genus_train <- filter(genus,genus$`Sample ID` %in% metadata[s,1])
          genus_query <- filter(genus,genus$`Sample ID` %in% metadata[setdiff(1:nrow(metadata),s),1])
          ...
          write_tsv(genus_train,paste0("/home/wangnan/GGMP-SGMP/exp_region/city_gradient/exp",n,"/",disease[i],"/data/",city[j,2],"/trainsource.tsv"),col_names = FALSE)
          write_tsv(genus_query,paste0("/home/wangnan/GGMP-SGMP/exp_region/city_gradient/exp",n,"/",disease[i],"/data/",city[j,2],"/querysource.tsv"),col_names = FALSE)
    
          #制作mapper文件
          metadata_train <- filter(metadata,metadata$SampleID %in% metadata[s,1])
          metadata_query <- filter(metadata,metadata$SampleID %in% metadata[setdiff(1:nrow(metadata),s),1])
          ...
          write_tsv(train_mapper,paste0("/home/wangnan/GGMP-SGMP/exp_region/city_gradient/exp",n,"/",disease[i],"/data/",city[j,2],"/trainmapper.tsv"))
          write_tsv(query_mapper,paste0("/home/wangnan/GGMP-SGMP/exp_region/city_gradient/exp",n,"/",disease[i],"/data/",city[j,2],"/querymapper.tsv"))
          ...
          biome <- biome[-1,]
          write_tsv(biome,paste0("/home/wangnan/GGMP-SGMP/exp_region/city_gradient/exp",n,"/",disease[i],"/data/",city[j,2],"/biome.tsv"),col_names = FALSE)
    

          #制作路径文件
          path_train <- tibble(path_ggmp_train = 1)
          path_query <- tibble(path_ggmp_query = 1)
          path_train <- rbind(path_train,paste0("/home/wangnan/GGMP-SGMP/exp_region/city_gradient/exp",n,"/",disease[i],"/data/",city[j,2],"/trainsource.tsv"))
          path_query <- rbind(path_query,paste0("/home/wangnan/GGMP-SGMP/exp_region/city_gradient/exp",n,"/",disease[i],"/data/",city[j,2],"/querysource.tsv"))
          path_train <- path_train[-1,]
          path_query <- path_query[-1,]
          write_tsv(path_train,paste0("/home/wangnan/GGMP-SGMP/exp_region/city_gradient/exp",n,"/",disease[i],"/data/",city[j,2],"/path_train.tsv"),col_names = FALSE)
          write_tsv(path_query,paste0("/home/wangnan/GGMP-SGMP/exp_region/city_gradient/exp",n,"/",disease[i],"/data/",city[j,2],"/path_query.tsv"),col_names = FALSE)
        }
      }
    }
